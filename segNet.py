...
class SegNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.fam3_1 = FAM(channel=256)

        self.conv3_2 = nn.Conv2d(256, 256, 3, padding=1)
        self.conv3_2_bn = nn.BatchNorm2d(256)

        self.fam3_2 = FAM(channel=256)
        ...
        self.fam4_1 = FAM(channel=512)

        self.conv4_2 = nn.Conv2d(512, 512, 3, padding=1)
        self.conv4_2_bn = nn.BatchNorm2d(512)

        self.fam4_2 = FAM(channel=512)
        ...
        self.fam5_1 = FAM(channel=512)

        self.conv5_2 = nn.Conv2d(512, 512, 3, padding=1)
        self.conv5_2_bn = nn.BatchNorm2d(512)

        self.fam5_2 = FAM(channel=512)
        ...
        self.fam5_D_1 = FAM(channel=512)

        self.conv5_2_D = nn.Conv2d(512, 512, 3, padding=1)
        self.conv5_2_D_bn = nn.BatchNorm2d(512)

        self.fam5_D_2 = FAM(channel=512)
        ...
        self.fam4_D_1 = FAM(channel=512)

        self.conv4_2_D = nn.Conv2d(512, 512, 3, padding=1)
        self.conv4_2_D_bn = nn.BatchNorm2d(512)

        self.fam4_D_2 = FAM(channel=512)
        ...
        self.fam3_D_1 = FAM(channel=256)

        self.conv3_2_D = nn.Conv2d(256, 256, 3, padding=1)
        self.conv3_2_D_bn = nn.BatchNorm2d(256)

        self.fam3_D_2 = FAM(channel=256)
    ...
    def forward(self, x, mask):
        ...
        x, fam_mask31 = self.fam3_1(x, mask)
        x = F.relu(self.conv3_2_bn(self.conv3_2(x)))
        x, fam_mask32 = self.fam3_2(x, mask)
        ...
        x, fam_mask41 = self.fam4_1(x, mask)
        x = F.relu(self.conv4_2_bn(self.conv4_2(x)))
        x, fam_mask42 = self.fam4_2(x, mask)
        ...
        x, fam_mask51 = self.fam5_1(x, mask)
        x = F.relu(self.conv5_2_bn(self.conv5_2(x)))
        x, fam_mask52 = self.fam5_2(x, mask)
        ...
        x, fam_mask5D1 = self.fam5_D_1(x, mask)
        x = F.relu(self.conv5_2_D_bn(self.conv5_2_D(x)))
        x, fam_mask5D2 = self.fam5_D_2(x, mask)
        ...
        x, fam_mask4D1 = self.fam4_D_1(x, mask)
        x = F.relu(self.conv4_2_D_bn(self.conv4_2_D(x)))
        x, fam_mask4D2 = self.fam4_D_2(x, mask)
        ...
        x, fam_mask3D1 = self.fam3_D_1(x, mask)
        x = F.relu(self.conv3_2_D_bn(self.conv3_2_D(x)))
        x, fam_mask3D2 = self.fam3_D_2(x, mask)
        ...
        x, fam_mask2D = self.fam2_D(x, mask)
        ...
        x, fam_mask1D = self.fam1_D(x, mask)
        x = self.conv1_1_D(x)
        x = self.sigmoid(x)
        return x, [fam_mask1, fam_mask2, fam_mask31, fam_mask32, fam_mask41, fam_mask42, fam_mask51, fam_mask52, fam_mask5D1, fam_mask5D2, fam_mask4D1, fam_mask4D2, fam_mask3D1, fam_mask3D2, fam_mask2D, fam_mask1D]
